...
class ImageProcessor:
    ...
    def load_and_resize_images(self, images) -> dict | list:
        """
        ...
        5) resize the image, keeping the ratio
        ...
        Args:
            images (dict): A dictionary of images, where the key is the depth and the value is the image
        Returns:
            dict: A dictionary with the depth as key and the base64 encoded image as value
        """
        encoded_images = []
    ...
    def resize_image(self, image) -> Image:
        """
        Resize the image to a new width, keeping the ratio.
        ...
        # To prevent 0px height - since it became an integer - Quick solution, it could be improved
        if aspect_ratio < 1:
    ...
    def apply_custom_colormap(self, image: Image) -> Image:
        ...
        colormap = [
            (int(255 * i / 255), 0, int(255 * (255 - i) / 255)) for i in range(256)
        ]
