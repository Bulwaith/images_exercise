...
class DbManager:
    ...
    def initialize_db(self):
        self.cur.execute(
            "CREATE TABLE IF NOT EXISTS images(depth INTEGER PRIMARY KEY, image_data TEXT)"
        )
    ...
    def get_image(self, depth: int):
        res = self.cur.execute(
            "SELECT depth, image_data FROM images WHERE depth = ?", [depth]
        )
        result = res.fetchone()
    ...
    def get_images(self, depth_min, depth_max):
        res = self.cur.execute(
            "SELECT depth, image_data FROM images WHERE depth BETWEEN ? AND ?",
            [depth_min, depth_max],
        )
        return res.fetchall()
