...
# This is a sample way to make any endpoint secure. - Only for the /add method
async def get_token(authorization: str = Header(None)):
    ...
    Args:
        authorization (str): The token passed in the header
    """
...
# GET endpoint to get a single image, as a PNG file, directly from the DB as a stream
@app.get("/image/")
def get_image(depth: int):
    """
    This endpoint expects a GET request, with a mandatory parameter: depth
    Args:
        depth (int): The depth of the image to fetch
    """
    try:
...
# GET endpoint to get a range of images, as base64 encoded strings
@app.get("/images")
def get_images(depth_min: int, depth_max: int):
    ...
    Args:
        depth_min (int): The minimum depth to fetch
        depth_max (int): The maximum depth to fetch
    """
    try:
        db = database_service.DbManager()
        ...
        if len(images) == 0:
            raise HTTPException(
                status_code=500, detail="No images found at the depths provided"
            )
        response = {
            "images_data": images,
            "depth_min": depth_min,
...
# Protected POST endpoint to upload a new CSV file
@app.post("/add")
async def add_images(
    csv_file: UploadFile = File(...), token: str = Security(get_token)
):
    """
    ...
    Args:
        ...
        token (str): The token to authorize the request
    """
    try:
...
def read_images_from_csv(csv_file):
    """
    Read the CSV file and load the images into a dictionary.
    It will also:
        - Check if each line is valid (correct number of non-empty fields, with pixel values between 0 and 255)
        - Return a dcistionary with depth->image_data structure and a list of invalid lines
    Args:
        csv_file (str): Path to the CSV file
    Returns:
        dict: A dictionary with depth as key and list of image data as value
        list: A list of indices of invalid lines
    """
    images = {}
    invalid_lines = []
...
def is_line_valid(line) -> bool:
    ...
    Args:
        line (list): A list of strings representing the fields of a CSV line
    Returns:
